...
void init_fail_msg_setup()
{
		...
		GetModuleFileNameA(nullptr, file_path, MAX_PATH))
	{
		...
		if (hash != (IS_LATEST_BUILD ? "754149fc8da2e131c2f13324c9e087f2a690f197" : "393ca001b796245e2d5425dd3505627810daecf8")) 
		{
			std::cout << "---------------> Unexpected portal2.exe hash. Hash was: " << hash.c_str() << std::endl;
...
DWORD WINAPI find_window_loop(LPVOID)
{
	...
	while (!glob::main_window)
	{
		...
		{
			init_fail_msg_setup();
			std::cout << "---------------> Failed to find main window with name: " << (IS_LATEST_BUILD ? "Portal 2 - Direct3D 9" : "PORTAL 2") << std::endl;
			init_fail_msg_post();
			return TRUE;
		}
	}
