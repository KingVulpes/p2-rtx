...
namespace components
{
	...
	public:
		...
		public:
			...
			//		Vector final_dir = m_attached_bone_forward; //(m_attached_bone_forward + offset_scale * world_offset);
			...
			//	if (is_attached())
			//	{
			//		const auto bone_dir = calculate_direction_for_point(point); 

			//		// apply position offset along final bone direction
			//		Vector finalPos = m_attached_position + point->position * bone_dir;
			//		return finalPos;
			//	}
		...
		bool update_static_remix_light(light* l, const map_settings::remix_light_settings_s::point_s* pt);
		bool update_remix_light(light* l);
		bool spawn_remix_light(light* l);
		...
		void destroy_map_light(light* l);
		...
		void increase_attachframe_counter() { ++m_attachframe_counter; }
		...
		// returns current attach frame count (m_attachframe_counter)
